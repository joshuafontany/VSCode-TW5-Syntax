 ...
 */
function repositoryRoot(nodePath) {
    const wiki = $tw.boot && $tw.boot.wikiPath;
...
Command.prototype.execute = function () {
    ...
    const wanted = this.params.filter((p) => p.indexOf("--") !== 0 && p !== "list");
    const unknown = wanted.filter((w) => names.indexOf(w) < 0);
    if (unknown.length) {
    ...
    for (const gate of running) {
        ...
        let said = "";
        try {
            said = child.execFileSync("npm", ["run", gate, "--silent"], { encoding: "utf8", cwd: root, stdio: ["ignore", "pipe", "pipe"] });
        }
        catch (e) {
            code = e.status || 1;
        ...
        const summary = lines.slice().reverse().filter((l) => /^\S+ {2,}\S/.test(l.trim()))[0];
