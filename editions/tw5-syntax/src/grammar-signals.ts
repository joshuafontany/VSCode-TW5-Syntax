 ...
 */
function fieldTypes(): Record<string, string> {
  const declared: Record<string, string> = {};
  const modules = $tw.Tiddler.fieldModules || {};
  for (const name of Object.keys(modules).sort()) {
    const field = modules[name];
    if (field.parse === $tw.utils.parseStringArray) declared[name] = "titles";
    else if (field.parse === $tw.utils.parseDate) declared[name] = "date";
    else if (field.editType) declared[name] = field.editType;
