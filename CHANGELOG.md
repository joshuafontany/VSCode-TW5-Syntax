...
names `v2.2.1` published and adds 256 — `MIGRATION.md` names every one, with what it stands as now.
...
2.2.1 painted by resemblance: it guessed what TiddlyWiki would probably do with a line. 3.0.0 asks
what TiddlyWiki's own parser builds, and paints that.

- **A mistake stays where you made it.** An unclosed tag, macro call or inline run stops at the
  blank line that ends its block, the way TiddlyWiki stops it. One typo no longer repaints the
  rest of the file, so everything below a mistake still reads true.
- **No false alarms.** Error colours appear only where TiddlyWiki refuses something out loud. A
  stray `&`, an older HTML tag, an unfamiliar tag name, a lone angle bracket and the inside of a
  comment all read as the ordinary text TiddlyWiki treats them as.
- **The quiet misreadings, corrected.** A pragma reads as one only at the top of a tiddler. Four
  dashes read as a hyphen and an em dash, because TiddlyWiki builds exactly that. Adjacent code
  runs pair as TiddlyWiki pairs them. A list item after a line that only closes an HTML element
  opens a fresh bullet. A self-closing `<svg …/>` closes. A line of only spaces ends a paragraph.
  Table cell marks, filtered transclusions and style blocks close where TiddlyWiki closes them.

### Your tiddlers speak their own language

- **A tiddler's `type` decides how its body reads.** A `.tid` typed JSON colours as JSON, CSS as
  CSS, JavaScript as JavaScript, and an `image/svg+xml` tiddler hands its body to the XML grammar.
- **A `.multids` file reads by its header.** One `type:` there governs every tiddler in the file,
  the way TiddlyWiki reads it.
- **Fenced code becomes real code.** A fenced block labelled `js`, `css`, `json` and some two dozen
  other languages hands its language to the editor, so comment toggling, brackets and word
  selection inside it follow that language.
- **A `toml` fence reads full TOML 1.1**, through a grammar this extension carries privately,
  since VS Code ships none.
- **A dictionary tiddler reads its values as wikitext**, whatever the key.

### Editing that meets you halfway

- **`[[` and `<<` close themselves** as you type.
- **A double-click takes a TiddlyWiki word**, so `$:/core/ui/EditTemplate` selects whole.
- **Folding leaves the blank line** after a block where it stands.
- **Bold reads bold and italic reads italic** in every theme that styles them: the grammar hands a
  theme the font style, not only a colour.

### How it looks in your theme

- **A filter reads as its parts.** In `{{{ -[tag<x>] }}}` the prefix, the brackets, the operator
  and the operand's brackets each carry their own role, so a filter reads like a small sentence.
- **A URL reads its scheme**, `https://` as one unit, in prose and inside links alike.
- **Struck text reads struck in every theme.** It dims like an aside in all 65 bundled themes, and
  strikes through wherever the theme supports it, where 2.2.1 made it look underlined or plain.
- **A transclusion reads as one object**, and every opener a reader meets — `{{`, `<<`, `<`,
  `<$`, `<%` — shares one family, so a theme rule written for one reaches the rest. `((x))`,
  which displays a variable's values, wears the name `<<x>>` wears.
- **A widget attribute names itself** in the scope inspector rather than reading "unrecognized".
...
- **127 scope names retire.** A `editor.tokenColorCustomizations` rule naming one of them stops
...
- **Some constructs look different on purpose.** A reader who liked 2.2.1's look does not get it
  back by upgrading; that is the definition this version number answers to.
- **`<<` and `[[` close themselves but match nothing.** Wikitext writes `<` and `[` unpaired all
  the time, and bracket matching kept pairing the wrong ones.

### What it cannot do, and says so

A grammar reads one line at a time, so some readings stay out of its reach: italic or code carried
across a line break, and an HTML tag carried across a blank line. This grammar keeps its line or
blank-line bound there on purpose — an unbounded carry would let one stray `//` repaint every
paragraph after it. Each such limit stands named in `$:/tw5-syntax/TextMateCeiling`, beside the kind
of reader that could close it.

### For contributors

Every reading above answers to TiddlyWiki's own parser under two named readers — the development
fork and the pinned 5.4.1 release — and the gates, ledgers and floors that hold it live beside the
