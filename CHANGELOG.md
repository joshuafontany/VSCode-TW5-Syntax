...
- Every divergence on TiddlyWiki's own tiddlers, traced. Over 387 of them nothing diverges
  unexplained: every span stands explained by 123 written rulings, and none by a number somebody
  wanted smaller. One ruling names a fault rather than an intention — a hardlinebreaks block
  emits no container node, so a scope over its content stands over text by construction. 21 of
  the 123 carry a `READER <version>` reason (tools/reader-scope.js), naming the ONE reader — this
...
- A RULING STANDS STALE ONLY WHERE EVERY RUN EXPLAINS NOTHING WITH IT. The five `overreach-check`
  ...
  what each run used and reports what none of them did. It runs as a gate and in CI.
